# Tutorial: https://mark-borg.github.io/blog/2016/tsne/ -------------------


# try the training with my data -------------------------------------------
...
packages<-c("Seurat", "dplyr", "colorRamps", "ggplot2", "tsne", "scatterplot3d", "rgl")
...
ProjectName<-"20180606_Stjude_3D"
genome<- "mm10"
Run_mito_filter = FALSE
output_prefix <-"20180606_Stjude_3D"
max_pcs <-46
# resolution_list <-c(0.6, 2.5)
resolution_list<-2.5
...
for(resolution in resolution_list){
  ...
  my_object<-RunTSNE(my_object, dims.use = 1:max_pcs, do.fast = TRUE, dim.embed = 3)
...
#   drawmyplot(geneList, my_object, name = paste(output_prefix, "dim",max_pcs, "res",resolution, sep = ""))
  ...
  rgl.snapshot(filename=paste(animation_dir,"/frame-",
