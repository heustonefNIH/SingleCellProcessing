...
swarmfile_name = "cellranger.text"
match_pattern = "_001.fastq.gz$"
...
with open(skippedsamplesfile_name, "w") as skippedsamples:
    skippedsamples.write(
        "The following sample IDs were not written to the swarm file:\n"
    )
...
# Loop through directory and collect *fastq.gz files
for sampleDir in sample_list:  # for each sample id
    ...
    for _, _, fastq_files in os.walk(sampleDir):
        fastq_list.extend(
            [x for x in fastq_files if x.endswith("fastq.gz")]
        )  # Now all the fastq files in sampleDir dir are in a list
        # Get batch IDs
        if len(fastq_list) > 0:
            ...
            sample_set = set(
                [re.match(".*(?=_S\d+_L\d+)", x).group(0) for x in fastq_list]
            )
            ...
            for sample in sample_set:
                ...
                print(sample)
                print(
                    "Fastq file name format in",
                    sample,
                    "is not interpretable by current version of MakeSwarmFile_cellranger-HU.py",
                )
