...
def create_system_command(file_directory, gct_file, cls_file, output_label, number_of_clusters):
	...
	for current_cluster in range(0, number_of_clusters):
		...
		system_command = "".join(["java -cp ./gsea-3.0.jar -Xms512m -Xmx8164m xtools.gsea.Gsea -res ./",gct_file," -cls ./",cls_file,"#",current_cluster,"_versus_REST -gmx msigdb.v6.1.symbols.gmt -collapse false -mode Max_probe -norm meandiv -nperm 1000 -permute phenotype -rnd_type no_balance -scoring_scheme weighted -rpt_label ",output_label,"_C",current_cluster,"_vs_REST -metric log2_Ratio_of_Classes -sort real -order descending -create_gcts false -create_svgs false -include_only_symbols true -make_sets true -median false -num 100 -plot_top_x 20 -rnd_seed timestamp -save_rnd_lists false -set_max 500 -set_min 15 -zip_report false -out ./ -gui false"])
